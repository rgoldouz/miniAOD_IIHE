...
void IIHEModuleJet::beginJob(){
	...
	addBranch("jet_bDiscriminator_pfCMVAV2");
	addBranch("jet_charge");
	addBranch("jet_electronEnergy");
	addBranch("jet_photonEnergy");
	addBranch("jet_UncorrJet_pt");
	addBranch("jet_PUValue");
	addBranch("jet_PUID");
	addBranch("jet_Area");
	addBranch("jets_jecUnc");
...
void IIHEModuleJet::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	for ( unsigned int i = 0; i <pfJetHandle_->size(); ++i) {
		...
		// new added
		store("jet_bDiscriminator_pfCMVAV2", pfjet->bDiscriminator("pfCombinedMVAV2BJetTags"));

		store("jet_charge", pfjet->charge());
		store("jet_electronEnergy", pfjet->electronEnergy());
		store("jet_photonEnergy", pfjet->photonEnergy());
		store("jet_UncorrJet_pt",pfjet->correctedJet("Uncorrected").pt());

		store("jet_PUValue", pfjet->userFloat("pileupJetId:fullDiscriminant"));
		store("jet_PUID", pfjet->userInt("pileupJetId:fullId"));

		jecUnc.setJetEta(pfjet->eta());
		jecUnc.setJetPt(pfjet->pt()); // here you must use the CORRECTED jet pt
		store("jets_jecUnc", jecUnc.getUncertainty(true));
