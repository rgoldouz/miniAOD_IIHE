...
void IIHEModuleTau::beginJob(){
	...
	addBranch("tau_genJet_deltaR");
	addBranch("tau_genJet_pt");
	...
	addBranch("tau_genJet_Available");
...
void IIHEModuleTau::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	for ( unsigned int i = 0; i <tauCollection_->size(); ++i) {
		...
		if(!(iEvent.isRealData())) {
			...
			store("tau_genJet_Available", tauni->genJet());
			float DR_tau_genjet = 999;
			if(tauni->genJet()) { DR_tau_genjet = deltaR(tauni->p4(), tauni->genJet()->p4()) ;}
			store("tau_genJet_deltaR", DR_tau_genjet);
			store("tau_genJet_pt", tauni->genJet() ? tauni->genJet()->pt() : -9);

		} else {

			store("tau_genJet_Available", -9);
			store("tau_genJet_deltaR", 999);
			store("tau_genJet_pt",-9);
		}
