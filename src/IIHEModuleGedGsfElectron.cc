...
IIHEModuleGedGsfElectron::IIHEModuleGedGsfElectron(const edm::ParameterSet& iConfig, edm::ConsumesCollector && iC): IIHEModule(iConfig){
	...
	elemvaValuesMap_nonTrig                     = iC.consumes<edm::ValueMap<float> >(iConfig.getParameter<edm::InputTag>("elemvaValuesMap_nonTrig"));
	elemvaCategoriesMap_nonTrig                 = iC.consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("elemvaCategoriesMap_nonTrig"));
	elemvaValuesMap_Trig                        = iC.consumes<edm::ValueMap<float> >(iConfig.getParameter<edm::InputTag>("elemvaValuesMap_Trig"));
	elemvaCategoriesMap_Trig                    = iC.consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("elemvaCategoriesMap_Trig"));
	elemvaValuesMap_GP                          = iC.consumes<edm::ValueMap<float> >(iConfig.getParameter<edm::InputTag>("elemvaValuesMap_GP"));;
	elemvaCategoriesMap_GP                      = iC.consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("elemvaCategoriesMap_GP"));
	elemvaValuesMap_HZZ                         = iC.consumes<edm::ValueMap<float> >(iConfig.getParameter<edm::InputTag>("elemvaValuesMap_HZZ"));;
	elemvaCategoriesMap_HZZ                     = iC.consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("elemvaCategoriesMap_HZZ"));

	eleMVATrigwp80IdMap = iC.consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("eleMVATrigIdMap")); 
	eleMVAnonTrigwp80IdMap = iC.consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("eleMVAnonTrigIdMap"));
	eleMVATrigwp90IdMap = iC.consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("eleMVATrigwp90IdMap"));
	eleMVAnonTrigwp90IdMap  = iC.consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("eleMVAnonTrigwp90IdMap"));
	eleMVAHZZwpLooseIdMap  = iC.consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("eleMVAHZZwpLooseIdMap"));
...
void IIHEModuleGedGsfElectron::beginJob(){
	...
	addBranch("gsf_Spring15_MVATrigwp80IdMap");
	addBranch("gsf_Spring15_MVAnonTrigwp80IdMap");
	addBranch("gsf_Spring15_MVATrigwp90IdMap");
	addBranch("gsf_Spring15_MVAnonTrigwp90IdMap");
	addBranch("gsf_Spring15_MVAHZZwpLooseIdMap");
	...
	addBranch("gsf_mvaval_nonTrig");
	addBranch("gsf_mvacat_nonTrig");
	addBranch("gsf_mvaval_Trig");
	addBranch("gsf_mvacat_Trig");
	addBranch("gsf_mvaval_GP");
	addBranch("gsf_mvacat_GP");
	addBranch("gsf_mvaval_HZZ");
	addBranch("gsf_mvacat_HZZ");
...
void IIHEModuleGedGsfElectron::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	edm::Handle<edm::ValueMap<bool>> MVATrigwp80IdMap;
	iEvent.getByToken(eleMVATrigwp80IdMap, MVATrigwp80IdMap);

	edm::Handle<edm::ValueMap<bool>> MVAnonTrigwp80IdMap;
	iEvent.getByToken(eleMVAnonTrigwp80IdMap, MVAnonTrigwp80IdMap);


	edm::Handle<edm::ValueMap<bool>> MVATrigwp90IdMap;
	iEvent.getByToken(eleMVATrigwp90IdMap, MVATrigwp90IdMap);

	edm::Handle<edm::ValueMap<bool>> MVAnonTrigwp90IdMap;
	iEvent.getByToken(eleMVAnonTrigwp90IdMap, MVAnonTrigwp90IdMap);


	edm::Handle<edm::ValueMap<bool>> MVAHZZwpLooseIdMap;    
	iEvent.getByToken(eleMVAHZZwpLooseIdMap, MVAHZZwpLooseIdMap);

	edm::Handle<edm::ValueMap<int>> mvaCategoriesMap_nonTrig;
	iEvent.getByToken(elemvaCategoriesMap_nonTrig, mvaCategoriesMap_nonTrig);

	edm::Handle<edm::ValueMap<int>> mvaCategoriesMap_Trig;
	iEvent.getByToken(elemvaCategoriesMap_Trig, mvaCategoriesMap_Trig);

	edm::Handle<edm::ValueMap<int>> mvaCategoriesMap_GP;
	iEvent.getByToken(elemvaCategoriesMap_GP, mvaCategoriesMap_GP);

	edm::Handle<edm::ValueMap<int>> mvaCategoriesMap_HZZ;
	iEvent.getByToken(elemvaCategoriesMap_HZZ, mvaCategoriesMap_HZZ);

	edm::Handle<edm::ValueMap<float>> mvaValuesMap_nonTrig;
	iEvent.getByToken(elemvaValuesMap_nonTrig, mvaValuesMap_nonTrig);

	edm::Handle<edm::ValueMap<float>> mvaValuesMap_Trig;
	iEvent.getByToken(elemvaValuesMap_Trig, mvaValuesMap_Trig);

	edm::Handle<edm::ValueMap<float>> mvaValuesMap_GP;
	iEvent.getByToken(elemvaValuesMap_GP, mvaValuesMap_GP);

	edm::Handle<edm::ValueMap<float>> mvaValuesMap_HZZ;
	iEvent.getByToken(elemvaValuesMap_HZZ, mvaValuesMap_HZZ);
	...
	for( unsigned int i = 0 ; i < electronCollection_->size() ; i++ ) {
		...
		//
		store("gsf_mvaval_nonTrig"   , (*mvaValuesMap_nonTrig).get(gsfref)            ) ;
		store("gsf_mvacat_nonTrig"   , (*mvaCategoriesMap_nonTrig).get(gsfref)            ) ;
		store("gsf_mvaval_Trig"      , (*mvaValuesMap_Trig).get(gsfref)            ) ;
		store("gsf_mvacat_Trig"      , (*mvaCategoriesMap_Trig).get(gsfref)            ) ;
		store("gsf_mvaval_GP"        , (*mvaValuesMap_GP).get(gsfref)            ) ;
		store("gsf_mvacat_GP"        , (*mvaCategoriesMap_GP).get(gsfref)            ) ;
		store("gsf_mvaval_HZZ"       , (*mvaValuesMap_HZZ).get(gsfref)            ) ;
		store("gsf_mvacat_HZZ"       , (*mvaCategoriesMap_HZZ ).get(gsfref)            ) ;
                store("gsf_Spring15_MVATrigwp80IdMap", (*MVATrigwp80IdMap).get(gsfref)            ) ;
                store("gsf_Spring15_MVAnonTrigwp80IdMap", (*MVAnonTrigwp80IdMap).get(gsfref)            ) ;
                store("gsf_Spring15_MVATrigwp90IdMap", (*MVATrigwp90IdMap).get(gsfref)            ) ;
                store("gsf_Spring15_MVAnonTrigwp90IdMap", (*MVAnonTrigwp90IdMap).get(gsfref)            ) ;
                store("gsf_Spring15_MVAHZZwpLooseIdMap", (*MVAHZZwpLooseIdMap).get(gsfref)            ) ;
